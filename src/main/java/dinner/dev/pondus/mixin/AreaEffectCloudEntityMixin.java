...
public abstract class AreaEffectCloudEntityMixin extends Entity {
    ...
    public Direction gravitychanger$getAppliedGravityDirection() {
        ...
        if(vehicle != null) {
            return PondusAPIFabric.getGravityDirection(vehicle);
        }

        return PondusAPIFabric.getGravityDirection((AreaEffectCloudEntity)(Object)this);
    }*/
//
//
//
...
//
    ...
    //          if(PondusFabric.config.worldVelocity)
...
//
    ...
    private void modify_move_multiply_0(Level instance, ParticleOptions particleData, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed, Operation<Void> original) {
        ...
        double e = this.getY();
        double l = this.getZ();
        Vec3 modify = RotationUtil.vecWorldToPlayer(d, e, l, PondusAPI.getGravityDirection(this));
        d = modify.x + (double) (Mth.cos(h) * k);
        e = modify.y;
        l = modify.z + (double) (Mth.sin(h) * k);
        modify = RotationUtil.vecPlayerToWorld(d, e, l, PondusAPI.getGravityDirection(this));
